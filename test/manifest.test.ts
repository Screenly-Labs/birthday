...
//
// Like Opening Hours, Birthday is a *settings* app: it carries a JSON Schema of
...
describe('signage-app.json manifest', () => {
  ...
  test('has a store-valid id slug', () => {
    expect(manifest.id).toBe('birthday')
  ...
  test('exposes the name/message/from fields', () => {
    const props = Object.keys(manifest.settings.properties)
    for (const field of ['name', 'message', 'from']) {
